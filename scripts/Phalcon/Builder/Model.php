...
//            $initialize['schema'] = $this->snippet->getThisMethod('setSchema', $schema);
        ...
        $initialize['source'] = $this->snippet->getThisMethod('setSource', $table);
        ...
        foreach ($fields as $field) {
            ...
            //查询字段注释
            $comment = $db->fetchOne('SELECT COLUMN_NAME,DATA_TYPE,COLUMN_COMMENT FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = "' . $table . '" AND table_schema = "' . $schema . '" AND column_name = "' . $field->getName().'"');
            $field->comment = $comment['COLUMN_COMMENT'];
